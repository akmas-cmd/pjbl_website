...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($judul) || empty($slug)) {
        ...
        $cek = $pdo->prepare("SELECT id FROM tragedi_bersejarah WHERE slug = ?");
        $cek->execute([$slug]);
        if ($cek->fetch()) {
            $error = 'Slug sudah digunakan, gunakan slug lain.';
        } else {
            $stmt = $pdo->prepare("
                INSERT INTO tragedi_bersejarah
                (judul, slug, lokasi, tahun, deskripsi, sub1_judul, sub1_isi, sub2_judul, sub2_isi, sub3_judul, sub3_isi, foto)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([$judul, $slug, $lokasi, $tahun, $deskripsi, $sub1_judul, $sub1_isi, $sub2_judul, $sub2_isi, $sub3_judul, $sub3_isi, $foto]);
            header('Location: data_pahlawan.php?tab=tragedi&status=tambah_ok');
            exit;
        }
    }
}
...
<head>
  ...
  <title>PatriotID - Tambah Tragedi Bersejarah</title>
...
<body>
  ...
  <div class="layout">
    ...
    <main class="main">
      <h1 class="page-title">Tambah Tragedi Bersejarah</h1>
        ...
        <form method="POST" action="">
          ...
          <div class="form-row-2">
            <div class="form-group">
              ...
              <input type="text" name="judul" value="<?= htmlspecialchars($_POST['judul'] ?? '') ?>" placeholder="contoh: Tragedi Lubang Buaya" required>
            </div>
            <div class="form-group">
              <label>Slug <span class="required">*</span></label>
              <input type="text" name="slug" id="slug" value="<?= htmlspecialchars($_POST['slug'] ?? '') ?>" placeholder="contoh: lubang-buaya" required>
          ...
          <div class="form-row-2">
            <div class="form-group">
              <label>Lokasi</label>
              <input type="text" name="lokasi" value="<?= htmlspecialchars($_POST['lokasi'] ?? '') ?>" placeholder="contoh: Jakarta">
            </div>
            <div class="form-group">
              <label>Tahun</label>
              <input type="text" name="tahun" value="<?= htmlspecialchars($_POST['tahun'] ?? '') ?>" placeholder="contoh: 1965">
            </div>
          </div>
          ...
          <div class="form-row-2">
            <div class="form-group">
              ...
              <input type="text" name="sub1_judul" value="<?= htmlspecialchars($_POST['sub1_judul'] ?? '') ?>" placeholder="contoh: Latar Belakang">
            </div>
            <div class="form-group">
              <label>Isi Sub Judul 1</label>
              <textarea name="sub1_isi" rows="3"><?= htmlspecialchars($_POST['sub1_isi'] ?? '') ?></textarea>
            </div>
          </div>
          ...
          <div class="form-row-2">
            <div class="form-group">
              ...
              <input type="text" name="sub2_judul" value="<?= htmlspecialchars($_POST['sub2_judul'] ?? '') ?>" placeholder="contoh: Puncak Peristiwa">
            </div>
            <div class="form-group">
          ...
          <div class="form-row-2">
            <div class="form-group">
              ...
              <input type="text" name="sub3_judul" value="<?= htmlspecialchars($_POST['sub3_judul'] ?? '') ?>" placeholder="contoh: Setelah Peristiwa">
            </div>
            <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="text" name="foto" value="<?= htmlspecialchars($_POST['foto'] ?? '') ?>" placeholder="contoh: lubang-buaya.png">
