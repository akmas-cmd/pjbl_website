...
if ($id <= 0) { header('Location: data_pahlawan.php?tab=tempat'); exit; }

$stmt = $pdo->prepare("SELECT * FROM tempat_bersejarah WHERE id = ? LIMIT 1");
$stmt->execute([$id]);
$t = $stmt->fetch();
if (!$t) { header('Location: data_pahlawan.php?tab=tempat'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama) || empty($slug)) {
        ...
        $cek = $pdo->prepare("SELECT id FROM tempat_bersejarah WHERE slug = ? AND id != ?");
        $cek->execute([$slug, $id]);
        if ($cek->fetch()) {
            $error = 'Slug sudah digunakan oleh data lain.';
        } else {
            $upd = $pdo->prepare("
                UPDATE tempat_bersejarah SET
                  nama=?, slug=?, lokasi=?, deskripsi=?,
                  sub1_judul=?, sub1_isi=?, sub2_judul=?, sub2_isi=?,
                  sub3_judul=?, sub3_isi=?, foto=?
                WHERE id=?
            ");
            $upd->execute([$nama, $slug, $lokasi, $deskripsi, $sub1_judul, $sub1_isi, $sub2_judul, $sub2_isi, $sub3_judul, $sub3_isi, $foto, $id]);
            header('Location: data_pahlawan.php?tab=tempat&status=edit_ok');
            exit;
        }
    }
...
<head>
  ...
  <title>PatriotID - Edit Tempat Bersejarah</title>
...
<body>
  ...
  <div class="layout">
    ...
    <main class="main">
      <h1 class="page-title">Edit Tempat Bersejarah</h1>
        ...
        <form method="POST">
          ...
          <div class="form-row-2">
            <div class="form-group">
              ...
              <input type="text" name="nama" value="<?= htmlspecialchars($t['nama']) ?>" required>
            </div>
            <div class="form-group">
